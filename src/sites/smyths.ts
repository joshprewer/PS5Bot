...
export default class Smyths implements Site {
  name: string = 'Smyths'
  productUrl: string = 'https://www.smythstoys.com/uk/en-gb/video-games-and-tablets/playstation-5/playstation-5-consoles/playstation-5-console/p/191259'
  ...
  async attemptPurchase(page: puppeteer.Page): Promise<void> {
    const [button] = await page.$x('//*[@id="cookieLoad"]/div/div/div[1]/div[4]/div[1]/button');
    if (button) {
      await button.click();
    }
    ...
    await fillForm(firstNameId, process.env.FIRST_NAME, page)
    await page.keyboard.press('Tab')
    ...
    const expiryMonth = process.env.CARD_EXPIRY.substr(0, 2)
